...
impl Day for Day07 {
  type Input = HashMap<String, Circuit>;
  type Output = u64;
  fn day() -> u32 { 7 }
  fn year() -> u32 { 2015 }

  fn parse_input(input: String) -> HashMap<String, Circuit> {
    ...
    let circuits = input.lines().map(|line| line.trim()).filter(|line| !line.is_empty())
      .map(|line| {
    ...
    let mut circuit_map = HashMap::<String, Circuit>::new();
    for c in circuits { circuit_map.insert(c.sink().clone(), c); }
    circuit_map
  }
  ...
  fn part2(circuit_map: &HashMap<String, Circuit>) -> u64 {
    let res1 = Self::part1(circuit_map);
    let b = "b".to_string();
    let new_b = Send(Const(res1), b.clone());
    let mut new_circuit_map = HashMap::<String, Circuit>::new();
    for (k, v) in circuit_map.iter() {
      new_circuit_map.insert(k.clone(), v.clone());
    }
    new_circuit_map.insert(b.clone(), new_b);
    Self::part1(&circuit_map)
  }
}

pub mod tests {
  use crate::aoc2015::day07::Day07;
  use crate::common::day::Day;
