...
def readInput(input: String): (Grid, Point) =
  var start: Point = Point(-1, -1)
...
def dumpPoints(grid: Grid, points: HashSet[Point]): Unit =
...
def walk(grid: Grid, start: Point, steps: Int): Long =
