...
case class Vertex(pos: Point, dir: Direction):
...
case class Grid(grid: Array[Array[String]]):
  ...
  val start = Point(0, grid.head.zipWithIndex.find(_._1 == ".").map(_._2).get)
  val end = Point(height - 1, grid.last.zipWithIndex.find(_._1 == ".").map(_._2).get)

  override def toString(): String =
    grid.map(_.mkString).mkString("\n")

  def apply(r: Int, c: Int) = grid(r)(c)
  def apply(p: Point) = grid(p.r)(p.c)
  def apply(v: Vertex) = grid(v.pos.r)(v.pos.c)
  def valid(p: Point) = 0 <= p.r && p.r < height && 0 <= p.c && p.c < width
...
def dump(grid: Grid, depths: HashMap[Point, Long]) =
  ...
  for r <- 0 until grid.height
    ...
    do
      if depths.contains(Point(r, c))
      then render(r)(c) = "%3d".format(depths(Point(r, c)))
      else render(r)(c) = "  " + grid(r, c)
  println {
    render.map(row => row.mkString).mkString("\n")
  }

def getNeighbors(grid: Grid, curr: Vertex, visited: Set[Point]) =
  curr.neighbors.filter:
    v =>
      !visited.contains(v.pos) && grid.moveable(v)

def part1(grid: Grid): Long =
  val depths = HashMap[Point, Int]()
  ...
  // then recurse.
  def dfs(start: Vertex, depth: Int, visited: Set[Point]): Long =
  ...
  def bfs =
    val queue = ArrayDeque[(Long, Vertex, Set[Point])]()
...
def dumpBranches(grid: Grid, nodes: Set[Point]) =
  val render = Array.fill(grid.height) { Array.fill(grid.width) { "" } }
  for r <- 0 until grid.height
      c <- 0 until grid.width
  do render(r)(c) =
    if nodes.contains(Point(r, c))
      then "O"
      else grid(r, c)
  println { render.map(row => row.mkString("")).mkString("\n") }

def dumpBranches2(grid: Grid, branches: Set[Point], paths: Set[Point]) =
  val render = Array.fill(grid.height) { Array.fill(grid.width) { "" } }
  for r <- 0 until grid.height
      c <- 0 until grid.width
  do render(r)(c) =
    if paths.contains(Point(r, c)) then "."
    else if branches.contains(Point(r, c)) then "O"
    else "#"
  println {
...
def part2(grid: Grid): Long =
  ...
  def getNeighbors(node: Point, visited: Set[Point]) =
  ...
  // our edges.
  def getEdges(nodes: Set[Point]): Map[Point, Set[(Long, Point)]] =
    val queue = ArrayDeque[(Long, Point)]()
    val visited = HashSet[Point]()
    val edges = HashMap[Point, HashSet[(Long, Point)]]()
    for node <- nodes do
      edges(node) = HashSet()
  ...
  given Ordering[(Long, Point, Set[Point])] = Ordering.by((_: (Long, Point, Set[Point]))._1)
  val queue = PriorityQueue[(Long, Point, Set[Point])]()
