...
case class Point(x: Int, y: Int):
  ...
  def manhattan(p: aoc2015.Point) = (x - p.x).abs + (y - p.y).abs
  def +(p: aoc2015.Point) = aoc2015.Point(x + p.x, y + p.y)
  def -(p: aoc2015.Point) = aoc2015.Point(x - p.x, y - p.y)


case class SensorReading(sensor: aoc2015.Point, beacon: aoc2015.Point):
  ...
  def render =
    ...
    val origin = aoc2015.Point(radius, radius)
...
def readInput(input: String): Array[SensorReading] =
  ...
  input.trim().split("\n").map(_.trim()).filter(_.nonEmpty).map {
    case line_re(sx, sy, bx, by) => SensorReading(
      sensor=aoc2015.Point(sx.toInt, sy.toInt),
      beacon=aoc2015.Point(bx.toInt, by.toInt)
    )
  }.toArray
