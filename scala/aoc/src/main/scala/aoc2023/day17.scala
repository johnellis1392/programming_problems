...
case class Vertex(pos: aoc2015.Point, dir: Direction, depth: Int = 0):
...
case class Grid(matrix: Array[Array[Int]]):
  ...
  def apply(p: aoc2015.Point) = matrix(p.r)(p.c)
  def apply(v: Vertex) = matrix(v.pos.r)(v.pos.c)

  def valid(p: aoc2015.Point): Boolean = 0 <= p.r && p.r < height && 0 <= p.c && p.c < width
  def valid(v: Vertex): Boolean = valid(v.pos)

  def points =
    for r <- 0 until height
        c <- 0 until width
      yield aoc2015.Point(r, c)
...
case class Cache[A](height: Int, width: Int, default: A):
  ...
  def apply(p: aoc2015.Point) = cache(p.r)(p.c)
  def apply(v: Vertex) = cache(v.pos.r)(v.pos.c)
  def update(p: aoc2015.Point, a: A) = cache(p.r)(p.c) = a
...
def part1(input: String): Long =
  ...
  val origin = aoc2015.Point(0, 0)
  val destination = aoc2015.Point(grid.height - 1, grid.width - 1)
  val minSteps = 0
  val maxSteps = 2
